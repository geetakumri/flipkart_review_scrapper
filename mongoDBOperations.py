...
logger = logger_class.get_logger('mongoDBOperations.py')
...
class MongoDBManagement:
    ...
    def __init__(self, username, password):
        ...
        try:
            ...
            self.url = "mongodb+srv://{}:{}@testcluster.vlr1p.mongodb.net/myFirstDatabase?retryWrites=true&w=majority".format(
    ...
    def stop_db(self):
        try:
            mongo_client = self.getMongoDBClientObject()
            self.closeMongoDBconnection(mongo_client)
        except Exception as e:
            raise Exception(
                f"(getResultToDisplayOnBrowser) - Something went wrong on getting result from database.\n" + str(e))
