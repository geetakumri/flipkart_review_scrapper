...
logger = logger_class.get_logger('flipkart.py')
...
def index():
    if request.method == 'POST':
        ...
        if free_status != True:
            ...
            free_status = True
        search_string = request.form['content']  # obtaining the search string entered in the form
        ...
        try:
            ...
            mongo_client = MongoDBManagement(username='mongodb', password='mongodb')
            scrapper_object.openUrl("https://www.flipkart.com/")
            logger.debug("Url hitted")
            scrapper_object.login_popup_handle()
            logger.debug("login popup handled")
            scrapper_object.searchProduct(searchString=search_string)
            logger.debug(f"Search begins for {search_string}")

            if mongo_client.isCollectionPresent(collection_name=search_string, db_name=db_name):
                response = mongo_client.findAllRecords(db_name=db_name, collection_name=search_string)
                ...
                if len(reviews) > expected_review:
                    ...
                    threadClass(expected_review=expected_review, searchString=search_string,
                                scrapper_object=scrapper_object, review_count=review_count)
                    logger.info("data saved in scrapper file")
                    return redirect(url_for('feedback'))
            else:
                threadClass(expected_review=expected_review, searchString=search_string,
                            scrapper_object=scrapper_object,
    ...
    try:
        ...
        if collection_name is not None:
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(),
                                               chrome_options=chrome_options)
            mongo_client = MongoDBManagement(username='mongodb', password='mongodb')
            rows = mongo_client.findAllRecords(db_name="Flipkart-Scrapper", collection_name=collection_name)
...
@app.route('/close_db', methods=['GET'])
@cross_origin()
def close_db():
    mongo_client = MongoDBManagement(username='mongodb', password='mongodb')
    mongo_client.stop_db()
